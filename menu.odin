...
solve_menu_content_height :: proc(how_many_items, how_many_dividers: int) -> f32 {
	...
	return(
		style.menu_padding * 2 * f32(how_many_dividers) +
		f32(how_many_items) * style.scale +
		f32(how_many_dividers) \
	)
}
...
begin_menu :: proc(
	...
	object.size = text_layout.size + ctx.style.text_padding * {1, 2}
		...
		if object_is_visible(object) {
			...
			kn.add_box(
				...
				paint = kn.fade(
					color,
					math.lerp(
						math.lerp(f32(0.5), f32(0.8), object.animation.hover),
						f32(1.0),
						object.animation.press,
					),
				),
			...
			kn.add_box(
				{{box.lo.x, box_center_y(box)}, {box.hi.x, box_center_y(box) + 1}},
				paint = text_color,
			)
...
begin_submenu :: proc(
	...
	object.size = label_text.size + ctx.style.text_padding * 2
	if begin_object(object) {
		...
		if object_is_visible(object) {
			...
			kn.add_box(
				...
				paint = kn.fade(
					...
					max(
						...
						f32(i32(.Focused in object.state.current)),
					) *
					0.75,
				),
			)
			...
			kn.add_arrow(
				box_center(get_box_cut_right(object.box, box_height(object.box))),
				5,
				1,
	...
	return begin_menu_with_activator(
		object,
		width,
		how_many_items,
		how_many_dividers,
...
begin_menu_with_activator :: proc(
	...
	size :=
		[2]f32 {
			max(width, box_width(activator.box)),
			solve_menu_content_height(how_many_items, how_many_dividers),
		} +
		get_current_style().menu_padding * 2
	box := solve_menu_box(activator.box, size, get_current_style().popup_margin, side, 0)
	if int(side) < 2 {
	...
	push_stack(&ctx.object_stack, activator)
	...
	if ctx.mouse_release_time._nsec < activator.click.press_time._nsec {
